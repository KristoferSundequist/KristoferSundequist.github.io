...
export const optimizer_pi = tf.train.adam(0.0003, 0.9, 0.999, 1e-5);
...
export function getEpisode(
    ...
    actor,
    critic
): { states; actionprobs; values; actions; rewards; advantages; discounted_rewards } {
    return tf.tidy(() => {
...
export function train(
    ...
    for (let i = 0; i < episodes; i++) {
        tf.tidy(() => {
            ...
            console.log("Episode:", i, "/", episodes, "Reward/EpisodeSize:", logger.getLatest());
    ...
    tf.tidy(() => {
        ...
        for (let i = 0; i < epochs; i++) {
            ...
            optimizer_pi.minimize(() => {
                ...
                // ACTION LOSS //
                /////////////////
                const new_actionprobs = actor.predict(states_batch) as tf.Tensor2D;
                const new_taken_actionprobs = badGather(new_actionprobs, action_batch);
                const entropy = tf
                    .sum(new_actionprobs.mul(new_actionprobs.add(1e-8).log()), 1)
                    .neg()
                    .mean()
                    .mul(beta);
            ...
            optimizer_vf.minimize(() => {
                ...
                // VALUE LOSS //
                ////////////////
                const new_values = (critic.predict(states_batch) as tf.Tensor2D).squeeze();
...
export async function agent_loop(iters: number) {
    ...
    for (let i = 0; i < iters; i++) {
        const state = tf.tensor([g.getState()]);
        const probs = actor.predict(state) as tf.Tensor1D;
...
export async function loadIndexed(actorstring: string, criticstring: string): Promise<void> {
    actor = (await tf.loadLayersModel(`indexeddb://${actorstring}`)) as tf.Sequential;
    critic = (await tf.loadLayersModel(`indexeddb://${criticstring}`)) as tf.Sequential;
